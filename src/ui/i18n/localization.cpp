...
constexpr Table chinese = {
    ...
    "勾选后，Before 使用 DLSSNR.Color；After 仍为现有 Post-ReShade FX。",
    "中心偏移会捕获完整的 DLSS5 输入帧；桥接不可用时自动回退到原本的 Pre-ReShade FX Before。",
...
constexpr Table english = {
    ...
    "When enabled, Before uses DLSSNR.Color; After remains the existing Post-ReShade FX result.",
    "Center remap captures the complete DLSS5 input frame and falls back to the original Pre-ReShade FX Before when the bridge is unavailable.",
    "DLSS5 diagnostics", "DLSS5 module", "Waiting", "Loaded", "Evaluate hooks", "Calls",
    "Region copies", "Last result", "Last error",
    "Center capture", "Ready", "Falling back", "Generation",
